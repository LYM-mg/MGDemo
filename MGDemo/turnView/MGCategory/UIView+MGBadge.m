...
#define kMGBadgeDefaultFont                ([UIFont boldSystemFontOfSize:9])
#define kMGBadgeDefaultMaximumBadgeNumber  99

static const CGFloat kMGBadgeDefaultRadius = 3.f;

@implementation UIView (MGBadge)

#pragma mark - MGBadgeView
- (void)showBadge {
    ...
    if (self.badgeCustomView) {
        ...
        CGFloat w = (self.badgeRadius ? : kMGBadgeDefaultRadius) * 2;
...
- (void)showBadgeWithValue:(NSUInteger)value {
    ...
    self.badge.text    = (value > kMGBadgeDefaultMaximumBadgeNumber ?
                          [NSString stringWithFormat:@"%@+", @(kMGBadgeDefaultMaximumBadgeNumber)] :
...
- (UILabel *)badge {
    ...
    if (!bLabel) {
        CGFloat width = kMGBadgeDefaultRadius * 2;
...
- (UIFont *)badgeFont {
    return objc_getAssociatedObject(self, _cmd) ?: kMGBadgeDefaultFont;
}
...
- (void)setBadgeCustomView:(UIView *)badgeCustomView
{
    ...
    objc_setAssociatedObject(self,
                             ...
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    if (self.badgeCustomView) {
        if (CGRectEqualToRect(badgeCustomView.frame, CGRectZero)) {
            [badgeCustomView layoutIfNeeded];
        }
